...
def generate_cover_with_retry(image_provider: AgnesImageProvider, prompt: str, output_path: Path) -> dict:
    last_error: Exception | None = None
    for _ in range(3):
        try:
            return image_provider.generate_cover(prompt, output_path)
...
我想先开心一下""",
    ...
    for spec in specs:
        ...
        music_response = {}
        if not song.audio_path.exists() or song.audio_path.stat().st_size < 1_000_000:
        ...
        image_response = generate_cover_with_retry(image_provider, spec["cover_prompt"], song.cover_raw_path)
